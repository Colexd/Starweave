...
import { customSplitRegex, filterResponseChunk } from '../utils/text.js'; // 新增：分段和过滤文本工具
import { convertFaces } from '../utils/face.js'; // 新增：处理表情工具
...
// 存储每个用户/群的消息缓冲和定时器
const chatMessageBuffers = new Map(); // Map<string, { messages: string[], timer: NodeJS.Timeout | null, e: any, use: string, forcePictureMode: boolean }>
const interruptionFlags = new Map(); // Map<string, boolean>
const pendingRequests = new Map(); // Map<string, string>
...
export class chatgpt extends plugin {
  ...
  async chatgpt (e) {
    ...
    // 如果有回复内容，将其添加到 prompt 前面
    if (replyContent) {
      prompt = replyContent + prompt
      logger.info(`[ChatGPT] 已将回复内容添加到prompt中，最终prompt: ${prompt}`)
    }
    ...
    if (!chatMessageBuffers.has(conversationKey)) {
        chatMessageBuffers.set(conversationKey, { messages: [], timer: null, e: null, use: use, forcePictureMode: forcePictureMode });
        logger.info(`[ChatGPT] 为 ${conversationKey} 创建新的消息缓冲。`);
    }

    const buffer = chatMessageBuffers.get(conversationKey);

    // 存储或更新最新的 e 对象、use 和 forcePictureMode
    buffer.e = e;
    buffer.use = use;
    ...
    if (buffer.timer) {
        ...
        logger.info(`[ChatGPT Debug] 重置定时器，对话键: ${conversationKey}`);
    }
    ...
    }else if (prompt.includes('?')||prompt.includes('？')||prompt.includes('吗')) {
        waitTime = 4000; // 包含"?"时等待4秒
        logger.info(`[ChatGPT Debug] 检测到用户输入"?"，等待时间3秒`);
    }else if(prompt.includes('...')||prompt.includes('。')){
      waitTime = 12000; // 包含"..."时等待12秒
      logger.info(`[ChatGPT Debug] 检测到用户输入"..."，等待时间10秒`);
    }//此段代码在既有问号又有...的情况时，会延长等待时间至10秒
    ...
    buffer.timer = setTimeout(async () => {
      ...
      const combinedPrompt = currentBuffer.messages.join(' '); // 使用空格连接多条消息
  ...
  async abstractChat (e, prompt, use, forcePictureMode = false, requestId = null) {
    ...
    if (e.source && (e.atme || e.atBot)) {
      try {
        ...
        if (replyMsg && replyMsg.length > 0) {
          let originalMessage = replyMsg[0]
          
          // 检查被回复消息中是否有图片
          if (originalMessage.message && Array.isArray(originalMessage.message)) {
            for (let segment of originalMessage.message) {
              if (segment.type === 'image' && Config.imgOcr) {
                try {
                  // 创建临时的消息对象来处理回复消息中的图片
                  let tempE = {
                    ...e,
                    message: [segment]
                  }
                  let replyImgOcrText = await getImageOcrText(tempE)
                  if (replyImgOcrText) {
                    prompt = prompt.replace('[图片]', `[图片内容: ${replyImgOcrText}]`)
                    logger.info(`[ChatGPT] 成功识别回复消息中的图片内容: ${replyImgOcrText}`)
                  }
                } catch (err) {
                  logger.warn('[ChatGPT] 识别回复消息图片失败:', err)
                }
              }
            }
          }
        ...
        logger.warn('[ChatGPT] 处理回复消息图片失败:', err)
      }
    }
    ...
    // 检查 prompt 是否包含时间查询，如果是，则加入当前实时时间作为上下文
    const timeQueryRegex = /(当前时间)/;
    if (timeQueryRegex.test(prompt)) {
      const currentTime = moment().format('YYYY年MM月DD日 HH:mm:ss');
      prompt = `【当前实时时间：${currentTime}】${prompt}`;
    ...
    try {
      ...
      let tempTextForFindingEmojis = response; // 使用一个临时变量来避免修改原始响应
      ...
      // 【新增】优先发送图片表情包，每张图片单独发送
      if (imagesToSend.length > 0) {
        const sendImage = Math.random() < 1; // 100% 概率发送图片
        if (sendImage) {
          //延迟一秒钟
      ...
      if (Config.ttsMode === 'azure' && (use === 'claude' || use === 'bing') && await AzureTTS.getEmotionPrompt(e)) {
        let ttsRoleAzure = userSetting.ttsRoleAzure
      ...
      // 新增：处理 [[定时 YY/MM/DD HH:MM:SS xxxxxx yyyyyy]] 标记并调度定时消息
      const scheduleRegex = /\[\[定时\s*(\d{4}\/\d{1,2}\/\d{1,2}\s*\d{2}:\d{2}:\d{2})\s*(\d+)\s*(.*)\]\]/
      const scheduleMatch = response.match(scheduleRegex)
      if (scheduleMatch) {
        const [, dateTimeStr, targetQQ, content] = scheduleMatch
        const targetTime = moment(dateTimeStr, 'YYYY/MM/DD HH:mm:ss')
        const currentTime = moment()
        const delay = targetTime.diff(currentTime)

        logger.info(`[定时调试] dateTimeStr: ${dateTimeStr}, targetTime: ${targetTime.toISOString()}, currentTime: ${currentTime.toISOString()}, delay: ${delay}`)

        if (delay > 0) {
          const botInstance = e.bot; // 捕获 e.bot 实例
          const chatgptInstance = this; // 捕获 chatgpt 插件实例
          const originalE = e; // 捕获原始的 e 对象，以便获取 self_id 等信息
          setTimeout(async (bot) => {
            try {
              const dummyE = {
                ...
                isPrivate: true,
                bot: bot,
                self_id: originalE.self_id // 使用原始 e 对象的 self_id
              };
              const systemPrompt = `【system】:定时提示触发，请根据上下文和提示内容"${content}"给出提示语句以提醒对方做什么事情。`;
              // 动态确定定时消息使用的模型，与主程序保持一致
              const userData = await getUserData(targetQQ); // 获取目标用户的配置
              ...
              if (chatMessage && chatMessage.text) {
                await bot.pickFriend(targetQQ).sendMsg(chatMessage.text); // 发送 AI 生成的回复
                logger.info(`[定时消息][成功] 已向 ${targetQQ} 发送 AI 生成的定时消息：${chatMessage.text}`);
              } else {
                await bot.pickFriend(targetQQ).sendMsg(`定时提醒触发：${content}`); // 如果AI没有回复，发送原始内容
                logger.warn(`[定时消息][警告] AI未生成回复，已向 ${targetQQ} 发送原始定时消息：${content}`);
              }
            } catch (error) {
              logger.error(`[定时消息][错误] 发送定时消息失败到 ${targetQQ}：`, error);
              try {
                await bot.pickFriend(targetQQ).sendMsg(`定时提醒失败：${content}`); // 失败时也告知用户
          ...
          await this.reply(`[定时提醒] 已为您设置定时提醒：将在 ${dateTimeStr} 向 ${targetQQ} 发送由AI生成的提示消息。`, true);
          logger.info(`[定时设置] 成功设置定时任务：${dateTimeStr}，目标QQ：${targetQQ}，内容：${content}，延迟：${delay}毫秒。`);
          response = response.replace(scheduleMatch[0], '').trim(); // 移除标记
        } else {
          await this.reply(`[定时提醒][失败] 定时时间已过，无法设置定时提醒。请检查时间格式并确保时间在未来。您输入的标记为：${scheduleMatch[0]}`, true);
          logger.warn(`[定时设置][失败] 定时时间已过，无法设置定时任务。您输入的标记为：${scheduleMatch[0]}，计算延迟：${delay}毫秒。`);
          // 移除标记，即使设置失败也应该移除
          response = response.replace(scheduleMatch[0], '').trim(); 
        }
      }
      ...
      if (response.includes('[[语音]]')) {
        ...
        return // 不再执行后续的文本或图片发送逻辑
      }
      //新增[[empty]]处理
      if (response.includes('[[empty]]')) {
        logger.warn('[[empty]]不回复', true)
        return // 不再执行后续的文本或图片发送逻辑
      }
      ...
      if (useTTS) {
        ...
        if (Config.alsoSendText || ttsResponse.length > parseInt(Config.ttsAutoFallbackThreshold)) {
          ...
          // 【修改】将这里原来直接发送 `responseText` 的逻辑改为分段发送
          let texts = customSplitRegex(response, e.isPrivate ? /\n\n/g : /(?<!\?)[。？\n](?!\?)/, e.isPrivate ? Infinity : 3)
          for (let originalSegmentT of texts) {
            ...
            if (textMsgArray.length > 0) {
              ...
              await new Promise((resolve) => {
                setTimeout(() => {
                  resolve();
                }, Math.min(originalSegmentT.length * 200, 3000)); // 文本长度决定延迟
              });
            }
          }
        ...
        // 原有的带 btnData 的回复也已移除，因为现在是分段发送
        // this.reply(responseText, e.isGroup, { btnData: { use, suggested: chatMessage.suggestedResponses } })
